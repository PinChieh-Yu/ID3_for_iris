...
int main(int argc, char** argv){
	...
	Node* Root;
	BuildDecisionTree(Root, TrainingDataSet);
...
void GetPerformance(vector<IrisData> set, Node* tree, double* ansset){
	...
	for(int i = 0; i < 3; i++){
		...
		for(int j = 0; j < set.size(); j++){
			...
			if(set[j].IrisClass == i && node->Result == i) prediction[0]++;
			else if(set[j].IrisClass != i && node->Result == i) prediction[1]++;
			else if(set[j].IrisClass == i && node->Result != i) prediction[2]++;
		}
